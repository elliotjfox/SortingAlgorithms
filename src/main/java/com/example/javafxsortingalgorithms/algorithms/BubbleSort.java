...
public class BubbleSort extends SortingAlgorithm {
    ...
    protected void runAlgorithm() {
        ...
        animation.setItemPosition(leftPointer, new ElementScaledPosition(0, 0));
        animation.setItemPosition(rightPointer, new ElementScaledPosition(1, 0));

        for (int i = 0; i < list.size(); i++) {
            hasSwapped = false;
            for (int j = 0; j < list.size() - 1 - i; j++) {
                // Move animation items
                animation.changeItemX(leftPointer, new ElementScaledIndex(j));
                animation.changeItemX(rightPointer, new ElementScaledIndex(j + 1));
                animation.addFrame();
